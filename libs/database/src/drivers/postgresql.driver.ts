 ...
 * PostgreSQL 数据库驱动
 *
 * @description 实现 PostgreSQL 数据库的连接和管理
 *
 * @since 1.0.0
 */
 ...
 * PostgreSQL 数据库驱动
 *
 * @description 实现 PostgreSQL 数据库的连接、查询和事务管理
 */
export class PostgreSQLDriver implements DatabaseDriver {
  ...
  async connect(): Promise<void> {
    try {
      this.logger.log("正在连接 PostgreSQL 数据库", {
      ...
      throw new Error(
        `PostgreSQL 连接失败: ${error instanceof Error ? error.message : "未知错误"}`,
      );
    }
  }
   ...
   * 断开数据库连接
   *
   * @description 优雅关闭 PostgreSQL 数据库连接
   */
  async disconnect(): Promise<void> {
    if (this.orm) {
      try {
        await this.orm.close();
        this.logger.log("PostgreSQL 数据库连接已关闭");
      } catch (error) {
        this.logger.error(error as Error);
      } finally {
        this.orm = null;
   ...
   * 检查连接状态
   *
   * @description 检查 PostgreSQL 数据库连接是否活跃
   *
   * @returns 连接状态
   */
   ...
   * 获取连接信息
   *
   * @description 获取当前 PostgreSQL 连接的详细信息
   *
   * @returns 连接信息
   */
   ...
   * 获取连接池统计
   *
   * @description 获取 PostgreSQL 连接池的使用统计信息
   *
   * @returns 连接池统计
   */
   ...
   * 健康检查
   *
   * @description 执行 PostgreSQL 数据库健康检查
   *
   * @returns 健康检查结果
   */
  ...
  getConfigSummary(): string {
    return `postgresql://${this.config.connection.host}:${this.config.connection.port}/${this.config.connection.database}`;
  }
}
