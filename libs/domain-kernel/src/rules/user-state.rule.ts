 ...
 * 用户状态业务规则验证器
 */
export class UserStateBusinessRule extends BusinessRuleValidator {
    ...
    if (userData.createdAt && userData.updatedAt) {
      if (userData.updatedAt < userData.createdAt) {
        errors.push({
          ...
          field: 'updatedAt',
          context: { createdAt: userData.createdAt, updatedAt: userData.updatedAt }
        });
      }
    }
  ...
  isApplicable(context: any): boolean {
    return context.operation === 'status_change' || context.operation === 'user_update';
  }
