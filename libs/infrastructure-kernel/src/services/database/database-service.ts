...
export class DatabaseService {
  ...
  async createConnection(
    ...
    try {
      ...
      throw new Error(`创建数据库连接失败: ${error instanceof Error ? error.message : '未知错误'}`);
    }
  }
  ...
  async closeConnection(name: string): Promise<void> {
    try {
      ...
      throw new Error(`关闭数据库连接失败: ${error instanceof Error ? error.message : '未知错误'}`);
    }
  }
  ...
  async executeOperation<T>(
    ...
    try {
      ...
      throw new Error(`执行数据库操作失败: ${error instanceof Error ? error.message : '未知错误'}`);
    }
  }
  ...
  async executeTransaction<T>(
    ...
    try {
      ...
      throw new Error(`执行事务失败: ${error instanceof Error ? error.message : '未知错误'}`);
    }
  }
  ...
  async getConnectionStatus(name: string): Promise<string> {
    try {
      ...
      throw new Error(`获取连接状态失败: ${error instanceof Error ? error.message : '未知错误'}`);
    }
  }
  ...
  async getAllConnectionStatus(): Promise<Record<string, string>> {
    try {
      ...
      throw new Error(`获取所有连接状态失败: ${error instanceof Error ? error.message : '未知错误'}`);
    }
  }
  ...
  async reconnect(name: string): Promise<void> {
    try {
      ...
      throw new Error(`重新连接失败: ${error instanceof Error ? error.message : '未知错误'}`);
    }
  }
  ...
  async reconnectAll(): Promise<void> {
    try {
      ...
      throw new Error(`重新连接所有连接失败: ${error instanceof Error ? error.message : '未知错误'}`);
    }
  }
   ...
   * 获取连接信息
   */
  async getConnectionInfo(connectionName: string): Promise<Record<string, any>> {
    try {
      const connection = await this.getConnection(connectionName);
  ...
  async batchInsert(
    ...
    try {
      ...
      throw new Error(`批量插入失败: ${error instanceof Error ? error.message : '未知错误'}`);
    }
  }
  ...
  getManagerStats(): Record<string, any> {
    try {
      ...
      throw new Error(`获取管理器统计信息失败: ${error instanceof Error ? error.message : '未知错误'}`);
    }
  }
