...
describe("DatabaseService - 真实实现", () => {
  ...
  describe("数据库连接管理", () => {
    ...
    it("应该能够关闭数据库连接", async () => {
      await service.createConnection(
        "test-db",
        TestDataFactory.createDatabaseConfig(),
      );
    ...
    it("应该处理连接错误", async () => {
      jest
        .spyOn(mockConnectionManager, "getConnection")
        .mockRejectedValue(new Error("连接失败"));

      await expect(service.getConnection("test-db")).rejects.toThrow(
        "获取数据库连接失败: 连接失败",
      );
    });
    ...
    it("应该处理创建连接错误", async () => {
      jest
        .spyOn(mockConnectionManager, "createConnection")
        .mockRejectedValue(new Error("创建失败"));

      await expect(service.createConnection("test-db", {})).rejects.toThrow(
        "创建数据库连接失败: 创建失败",
      );
    });
  });
  ...
  describe("健康检查", () => {
    ...
    it("应该处理健康检查失败", async () => {
      jest
        .spyOn(mockConnectionManager, "healthCheck")
        .mockRejectedValue(new Error("健康检查失败"));
  ...
  describe("连接池管理", () => {
    it("应该能够配置连接池", async () => {
      ...
      await expect(
        service.configureConnectionPool("test-db", config),
  ...
  describe("隔离上下文支持", () => {
    it("应该能够应用隔离上下文", async () => {
      ...
      const serviceWithContext = new DatabaseService(
  ...
  describe("错误处理", () => {
    it("应该处理无效的连接名称", async () => {
      jest
        .spyOn(mockConnectionManager, "getConnection")
        .mockRejectedValue(new Error("连接不存在"));

      await expect(service.getConnection("invalid-db")).rejects.toThrow(
        "获取数据库连接失败: 连接不存在",
      );
    });
    ...
    it("应该处理连接超时", async () => {
      ...
      await expect(service.getConnection("timeout-db")).rejects.toThrow(
        "获取数据库连接失败: 连接超时",
      );
    });
