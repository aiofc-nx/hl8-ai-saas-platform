...
describe("DatabaseService - 改进版", () => {
  ...
  describe("数据库连接", () => {
    ...
    it("应该能够处理连接错误", async () => {
      ...
      jest
        .spyOn(connectionManager, "createConnection")
    ...
    it("应该能够处理断开连接错误", async () => {
      const connectionManager = module.get("IDatabaseConnectionManager");
      jest
        .spyOn(connectionManager, "closeConnection")
  ...
  describe("健康检查", () => {
    ...
    it("应该处理健康检查失败", async () => {
      const connectionManager = module.get("IDatabaseConnectionManager");
      jest
        .spyOn(connectionManager, "healthCheck")
  ...
  describe("查询执行", () => {
    ...
    it("应该能够处理查询错误", async () => {
      ...
      jest
        .spyOn(connectionManager, "getConnection")
        .mockResolvedValue(mockConnection);

      await expect(
        service.query("test", "SELECT * FROM users"),
  ...
  describe("事务管理", () => {
    ...
    it("应该能够处理事务回滚", async () => {
      await expect(
        service.transaction("test", async (trx) => {
...
describe("ConnectionPoolService - 改进版", () => {
  ...
  describe("连接获取", () => {
    ...
    it("应该能够释放连接到连接池", async () => {
      const connection = await service.getConnection("test");
      await expect(
        service.releaseConnection("test", connection),
      ).resolves.not.toThrow();
    });
    ...
    it("应该能够处理连接池满的情况", async () => {
      ...
      jest
        .spyOn(connectionManager, "getConnection")
        .mockRejectedValue(new Error("连接池已满"));
...
describe("TransactionService - 改进版", () => {
  ...
  describe("事务管理", () => {
    ...
    it("应该能够处理事务执行错误", async () => {
      await expect(
        service.executeTransaction("test", async (trx) => {
  ...
  describe("错误处理", () => {
    ...
    it("应该处理连接错误", async () => {
      ...
      await expect(service.beginTransaction("test")).rejects.toThrow(
        "连接失败",
      );
    });
  });
