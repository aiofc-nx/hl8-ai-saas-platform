...
class MockConnectionPoolService {
  ...
  getPoolConfig() {
    return { minConnections: 2, maxConnections: 10 };
  }
  getPoolStats() {
    return { totalConnections: 5, activeConnections: 2 };
  }
}
...
class MockTransactionService {
  ...
  async beginTransaction() {
    return Promise.resolve({ id: "tx1", status: "ACTIVE" });
  }
...
class InfrastructureKernelService {
  ...
  async healthCheck() {
    ...
    try {
      results.applicationKernelIntegration =
    ...
    try {
      results.domainKernelIntegration =
  ...
  createIsolationContext(
    ...
    return this.isolationManager.createIsolationContext(
      tenantId,
      orgId,
      deptId,
      userId,
...
describe("InfrastructureKernelService - 简化版", () => {
  ...
  beforeEach(async () => {
    ...
    service = moduleRef.get<InfrastructureKernelService>(
  ...
  describe("初始化", () => {
    ...
    it("应该处理初始化失败", async () => {
      const databaseService =
        module.get<MockDatabaseService>(MockDatabaseService);
      jest
        .spyOn(databaseService, "initialize")
  ...
  describe("健康检查", () => {
    ...
    it("应该处理健康检查失败", async () => {
      const databaseService =
        module.get<MockDatabaseService>(MockDatabaseService);
      jest
        .spyOn(databaseService, "healthCheck")
        .mockRejectedValue(new Error("数据库健康检查失败"));
  ...
  describe("服务管理", () => {
    it("应该能够获取服务", () => {
      const databaseService =
        service.getService<MockDatabaseService>("database");
  ...
  describe("错误处理", () => {
    it("应该处理服务初始化错误", async () => {
      ...
      jest
        ...
        .mockRejectedValue(new Error("缓存初始化失败"));
    ...
    it("应该处理健康检查错误", async () => {
      const isolationManager =
        module.get<MockIsolationManager>(MockIsolationManager);
      jest
        .spyOn(isolationManager, "healthCheck")
        .mockRejectedValue(new Error("隔离管理器健康检查失败"));
