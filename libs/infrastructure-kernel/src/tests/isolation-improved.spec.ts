...
describe('IsolationManager - 改进版', () => {
  ...
  describe('访问控制', () => {
    it('应该能够验证访问权限', async () => {
      const context = service.createIsolationContext('tenant1');
      const hasAccess = await service.validateAccess(context, { id: 'resource1' });
  ...
  describe('健康检查', () => {
    ...
    it('应该处理健康检查失败', async () => {
      const contextManager = module.get<IsolationContextManager>(IsolationContextManager);
      jest.spyOn(contextManager, 'healthCheck').mockRejectedValue(new Error('上下文管理器健康检查失败'));
  ...
  describe('审计日志', () => {
    it('应该能够记录访问日志', async () => {
      ...
      await expect(service.logAccess(context, resource, 'READ')).resolves.not.toThrow();
    });
    ...
    it('应该能够记录数据访问日志', async () => {
      ...
      await expect(service.logDataAccess(context, data, 'SELECT')).resolves.not.toThrow();
    });
  });
  ...
  describe('安全监控', () => {
    it('应该能够监控访问行为', async () => {
      ...
      await expect(service.monitorAccess(context, resource, 'READ')).resolves.not.toThrow();
    });
  ...
  describe('错误处理', () => {
    it('应该处理无效的隔离上下文', () => {
      ...
      expect(() => service.setCurrentIsolationContext(invalidContext)).not.toThrow();
    });

    it('应该处理访问控制错误', async () => {
      const accessControlService = module.get<AccessControlService>(AccessControlService);
      jest.spyOn(accessControlService, 'validateAccess').mockRejectedValue(new Error('访问控制验证失败'));

      const context = service.createIsolationContext('tenant1');
      await expect(service.validateAccess(context, { id: 'resource1' })).rejects.toThrow('访问控制验证失败');
    });
  });
});
...
describe('IsolationContextManager - 改进版', () => {
  ...
  describe('隔离上下文创建', () => {
    ...
    it('应该能够创建用户级隔离上下文', () => {
      const context = service.createContext('tenant1', 'org1', 'dept1', 'user1');
      expect(context.tenantId).toBe('tenant1');
      expect(context.organizationId).toBe('org1');
      expect(context.departmentId).toBe('dept1');
      expect(context.userId).toBe('user1');
...
describe('AccessControlService - 改进版', () => {
  ...
  describe('权限验证', () => {
    it('应该能够验证读取权限', async () => {
      ...
      const hasReadAccess = await service.validateAccess(context, { id: 'resource1' }, 'READ');
    ...
    it('应该能够验证写入权限', async () => {
      ...
      const hasWriteAccess = await service.validateAccess(context, { id: 'resource1' }, 'WRITE');
    ...
    it('应该能够验证删除权限', async () => {
      ...
      const hasDeleteAccess = await service.validateAccess(context, { id: 'resource1' }, 'DELETE');
