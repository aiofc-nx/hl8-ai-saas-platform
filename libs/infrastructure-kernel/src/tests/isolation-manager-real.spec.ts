...
describe('IsolationManager - 真实实现', () => {
  ...
  describe('隔离上下文管理', () => {
    ...
    it('应该能够设置当前隔离上下文', () => {
      ...
      expect(mockContextManager.setCurrentContext).toHaveBeenCalledWith(context);
    });
  ...
  describe('访问控制', () => {
    it('应该能够验证访问权限', async () => {
      ...
      expect(mockAccessControl.validateAccess).toHaveBeenCalledWith(context, resource);
    });
    ...
    it('应该能够应用隔离过滤', () => {
      ...
      expect(mockAccessControl.applyIsolationFilter).toHaveBeenCalledWith(query, context);
    });
    ...
    it('应该拒绝跨租户访问', async () => {
      const context = TestDataFactory.createIsolationContext({ tenantId: 'tenant1' });
  ...
  describe('健康检查', () => {
    ...
    it('应该处理健康检查失败', async () => {
      jest.spyOn(mockContextManager, 'healthCheck').mockRejectedValue(new Error('上下文管理器健康检查失败'));
  ...
  describe('数据隔离测试', () => {
    it('应该正确隔离不同租户的数据', () => {
      const tenant1Context = TestDataFactory.createIsolationContext({ tenantId: 'tenant1' });
      const tenant2Context = TestDataFactory.createIsolationContext({ tenantId: 'tenant2' });
      ...
      expect(tenant1Data.every(item => item.tenantId === 'tenant1')).toBe(true);
      
      expect(tenant2Data).toHaveLength(1);
      expect(tenant2Data.every(item => item.tenantId === 'tenant2')).toBe(true);
    });
    ...
    it('应该正确隔离不同组织的数据', () => {
      ...
      const data = [
        { id: '1', tenantId: 'tenant1', organizationId: 'org1', name: 'User 1' },
        { id: '2', tenantId: 'tenant1', organizationId: 'org2', name: 'User 2' },
        { id: '3', tenantId: 'tenant1', organizationId: 'org1', name: 'User 3' }
      ];
      ...
      expect(org1Data.every(item => item.organizationId === 'org1')).toBe(true);
      
      expect(org2Data).toHaveLength(1);
      expect(org2Data.every(item => item.organizationId === 'org2')).toBe(true);
    });
    ...
    it('应该正确隔离不同部门的数据', () => {
      ...
      const data = [
        { id: '1', tenantId: 'tenant1', organizationId: 'org1', departmentId: 'dept1', name: 'User 1' },
        { id: '2', tenantId: 'tenant1', organizationId: 'org1', departmentId: 'dept2', name: 'User 2' },
        { id: '3', tenantId: 'tenant1', organizationId: 'org1', departmentId: 'dept1', name: 'User 3' }
      ];
      ...
      expect(dept1Data.every(item => item.departmentId === 'dept1')).toBe(true);
      
      expect(dept2Data).toHaveLength(1);
      expect(dept2Data.every(item => item.departmentId === 'dept2')).toBe(true);
    });
    ...
    it('应该正确隔离不同用户的数据', () => {
      ...
      const data = [
        { id: '1', tenantId: 'tenant1', organizationId: 'org1', departmentId: 'dept1', userId: 'user1', name: 'User 1' },
        { id: '2', tenantId: 'tenant1', organizationId: 'org1', departmentId: 'dept1', userId: 'user2', name: 'User 2' },
        { id: '3', tenantId: 'tenant1', organizationId: 'org1', departmentId: 'dept1', userId: 'user1', name: 'User 3' }
      ];
