...
class IntegrationTestService {
  ...
  private registerServices() {
    ...
    this.services.set('isolation', {
      initialize: jest.fn().mockResolvedValue(undefined),
      healthCheck: jest.fn().mockResolvedValue(true),
      createContext: jest.fn().mockImplementation((tenantId, orgId, deptId, userId) => ({
  ...
  async testIsolationIntegration() {
    ...
    const context = isolation.createContext('tenant1', 'org1', 'dept1', 'user1');
    const hasAccess = await isolation.validateAccess(context, { id: 'resource1' });
  ...
  async testFullWorkflow() {
    ...
    // 3. 执行数据库查询
    const queryResult = await db.query('SELECT * FROM users WHERE tenant_id = $1', [context.tenantId]);
...
describe('基础设施层集成测试', () => {
  ...
  describe('完整工作流程', () => {
    ...
    it('应该能够处理错误情况', async () => {
      ...
      await expect(service.testDatabaseIntegration()).rejects.toThrow('数据库错误');
    });
  });
  ...
  describe('错误处理', () => {
    it('应该处理服务初始化错误', async () => {
      const db = service.getService('database');
      jest.spyOn(db, 'initialize').mockRejectedValue(new Error('数据库初始化失败'));
      
      await expect(service.initialize()).rejects.toThrow('集成测试服务初始化失败: 数据库初始化失败');
    });
    ...
    it('应该处理健康检查错误', async () => {
      ...
      const cache = service.getService('cache');
      jest.spyOn(cache, 'healthCheck').mockRejectedValue(new Error('缓存健康检查失败'));
    ...
    it('应该处理服务调用错误', async () => {
      ...
      const isolation = service.getService('isolation');
      jest.spyOn(isolation, 'validateAccess').mockRejectedValue(new Error('访问验证失败'));
      
      await expect(service.testIsolationIntegration()).rejects.toThrow('访问验证失败');
    });
  });
});
