...
async function testEntityMapping() {
  ...
  try {
    // 动态导入实体映射器
    const { EntityMapper } = await import('/home/arligle/hl8/hl8-ai-saas-platform/libs/database/dist/index.js');
...
async function testMetricsService() {
  ...
  try {
    // 动态导入指标服务
    const { MetricsService } = await import('/home/arligle/hl8/hl8-ai-saas-platform/libs/database/dist/index.js');
...
async function testDatabaseDriverFactory() {
  ...
  try {
    // 动态导入数据库驱动工厂
    const { DatabaseDriverFactory } = await import('/home/arligle/hl8/hl8-ai-saas-platform/libs/database/dist/index.js');
...
async function runHl8DatabaseTests() {
  ...
  console.log(`指标服务:     ${results.metricsService ? '✅ 通过' : '❌ 失败'}`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  runHl8DatabaseTests().then(success => {
...
export { runHl8DatabaseTests, testHl8Database, testEntityMapping, testMetricsService, testDatabaseDriverFactory };
