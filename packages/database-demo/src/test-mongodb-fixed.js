...
async function testMongoDBAggregation() {
  ...
  try {
    ...
    categoryStats.forEach(stat => {
      console.log(`  - ${stat._id}: ${stat.totalProducts} 个商品, 平均价格: ${stat.avgPrice.toFixed(2)}, 总库存: ${stat.totalStock}`);
    });
...
async function testHl8DatabaseMongoDBSimple() {
  ...
  try {
    ...
    if (fs.existsSync(configPath)) {
      ...
      if (configContent.includes('mongodb') || configContent.includes('MongoDB')) {
    ...
    if (fs.existsSync(typesPath)) {
      ...
      const mongoTypeFiles = typeFiles.filter(file => 
...
if (import.meta.url === `file://${process.argv[1]}`) {
  runMongoDBFixedTests().then(success => {
...
export { runMongoDBFixedTests, testMongoDBBasic, testMongoDBAggregation, testMongoDBPerformance, testHl8DatabaseMongoDBSimple, testMongoDBMonitoring };
