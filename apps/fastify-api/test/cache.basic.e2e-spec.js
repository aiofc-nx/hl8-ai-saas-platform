 ...
 * 缓存基础端到端测试
 *
 * @description 测试缓存模块的基本端到端功能，使用 JavaScript 避免 TypeScript 编译问题
 */

describe("缓存基础端到端测试", () => {
  describe("缓存API端点基本测试", () => {
    it("应该支持平台级缓存API", async () => {
      const key = "platform-basic-test";
    ...
    it("应该支持组织级缓存API", async () => {
      const key = "organization-basic-test";
      const headers = {
        "X-Tenant-Id": "tenant-123",
  ...
  describe("多级数据隔离基本测试", () => {
    it("应该在不同隔离级别间保持数据隔离", async () => {
      const key = "isolation-basic-test";
  ...
  describe("性能监控基本测试", () => {
    it("应该返回缓存性能指标", async () => {
      // 执行一些缓存操作以生成指标
      await fetch("http://localhost:3000/cache/platform/metrics-basic-test-1");
      await fetch("http://localhost:3000/cache/platform/metrics-basic-test-1"); // 缓存命中
      await fetch("http://localhost:3000/cache/platform/metrics-basic-test-2");
  ...
  describe("并发处理基本测试", () => {
    it("应该正确处理并发缓存请求", async () => {
      const promises = Array.from({ length: 5 }, (_, i) =>
        fetch(`http://localhost:3000/cache/platform/concurrent-basic-test-${i}`)
      );
