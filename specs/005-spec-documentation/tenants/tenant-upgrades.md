...
```typescript
async function upgradeImmediate(tenant: TenantAggregate, newType: TenantType) {
...
```typescript
async function upgradeEndOfCycle(tenant: TenantAggregate, newType: TenantType) {
  // 1. Schedule upgrade
  await upgradeScheduler.schedule(tenant.id, newType, tenant.billingCycleEnd);
...
async function downgradeEndOfCycle(
  ...
  await downgradeScheduler.schedule(tenant.id, newType, tenant.billingCycleEnd);
...
async function disablePremiumFeatures(
  ...
  const disabledFeatures = currentFeatures.filter(
